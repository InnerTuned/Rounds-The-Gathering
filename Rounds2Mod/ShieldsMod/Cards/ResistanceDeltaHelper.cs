...
internal static class ResistanceDeltaHelper
{
    ...
    internal static IReadOnlyList<(string label, string before, string after)> ComputeTypeUpgradeDelta(
        Player player,
        ResistanceType type,
        CardInfo excludeCard = null)
    {
        if (player == null)
            return Array.Empty<(string, string, string)>();

        var survivors = player.data?.currentCards?
            .Where(c => c != null && !ReferenceEquals(c, excludeCard)) ?? Enumerable.Empty<CardInfo>();

        int tier = ResistanceHandLogic.GetNextUpgradeTier(survivors, type, excludeCard);
        string statLabel = ResistanceCardCatalog.GetStatLabel(type);
        float beforePct = ResistanceCardCatalog.GetComputeFromHand(type)(survivors) * 100f;
        float before = beforePct / 100f;
        float afterPct = ResistanceTierLogic.ApplyCard(before, tier, out float? healthMult) * 100f;
        ...
        Func<IEnumerable<CardInfo>, float> computeFromHand)
    {
        ...
        var survivors = player.data?.currentCards?
            .Where(c => c != null && !ReferenceEquals(c, cardToRemove)) ?? Enumerable.Empty<CardInfo>();
