...
public class PoisonResistanceManager : MonoBehaviour
{
    ...
    public void RebuildForPlayer(int playerID)
    {
        ...
        float reduction = PoisonHandRebuild.ComputeReductionFromHand(player.data.currentCards);
        _reductionByPlayer[playerID] = reduction;
        SLog.Line($"PoisonResistance rebuild player={playerID} reduction={reduction * 100f:F0}%");
    }
